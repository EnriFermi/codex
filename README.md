...
Введите `/` в поле сообщения: появятся команды с описаниями. `↑` / `↓` выбирает пункт, `Tab` или `Enter` подставляет его, следующее `Enter` выполняет. Для `/theme` доступны подсказки названий тем; `/resume` открывает выбор беседы. `Esc` закрывает меню, сохраняя ввод. Вне поля сообщения `/` по-прежнему ищет по трассе.
...
Введите **`/resume` без UUID** или `/sessions`. Откроется список с названиями, датами, папками и превью. Начните печатать название или слова из превью, выберите беседу стрелками и нажмите Enter либо щёлкните по ней. Кнопка **All projects / This project** переключает область поиска. Старые страницы списка подгружаются автоматически, включая сессии других провайдеров; экран показывает прогресс загрузки. Esc отменяет выбор. Прямой `/resume UUID` тоже работает.
