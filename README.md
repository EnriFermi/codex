...
Введите `/` в поле сообщения: появятся команды с описаниями. `↑` / `↓` выбирает пункт, `Tab` или `Enter` подставляет его, следующее `Enter` выполняет. Для `/theme` доступны подсказки названий тем; для `/resume` и `/export` — подсказки аргументов. `Esc` закрывает меню, сохраняя ввод. Вне поля сообщения `/` по-прежнему ищет по трассе.
